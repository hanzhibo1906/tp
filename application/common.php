...
function setSalt()
{
	...
	return substr(str_shuffle($pool),rand(1,50),5);
}
//生成密码  salt盐值
function crestePwd($pwd,$salt)
{
...
//图片上传
	function upload($img)
	{
	    ...
	    if($info){
	        ...
	        // 上传失败获取错误信息
	        return json_encode(['code'=>1,'msg'=>$info->getError()]);
	    }
	}
...
function sendEmail($address,$code){
    ...
    $mail->Username ='18732861937@163.com';

    //smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
    $mail->Password = 'admin123';//163邮箱也有授权码 进入163邮箱帐号获取

    //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
    $mail->From = '18732861937@163.com';
...
/**发送邮件 */
function updateEmail($Subject,$body,$address){
    //实例化PHPMailer核心类
    $mail =new \email\PHPMailer();
